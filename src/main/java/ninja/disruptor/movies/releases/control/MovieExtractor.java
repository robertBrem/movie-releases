...
public class MovieExtractor {
    ...
    public List<Movie> getTop100Range30Days() {
        List<Movie> movies = new ArrayList<>();
        for (int pageIndex = 1; pageIndex <= 5; pageIndex++) {
            movies.addAll(getMoviesForPage(pageIndex));
        }
        return movies;
    }

    public List<Movie> getMoviesForPage(int pageIndex) {
        List<Movie> movies = new ArrayList<>();
    ...
    public Document getDocument(int pageIndex) {
        Document doc = null;
        try {
            doc = Jsoup
